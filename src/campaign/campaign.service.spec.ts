...
describe('CampaignService', () => {
  ...
  describe('findMany', () => {
    it('조건에 맞는 캠페인을 찾고 결과를 리턴한다.', async () => {
      ...
      // when
      const result = await campaignService.findMany({ city: 'city1', size: 10, page: 1 });
    ...
    it('조건에 맞지 않는 캠페인은 목록에서 제외한다.', async () => {
      ...
      // when
      const result = await campaignService.findMany({ city: 'city1', title: 'title2', size: 10, page: 1 });
    ...
    it('삭제된 캠페인이 존재할 경우 목록에서 제외한다', async () => {
      await prismaService.campaign.create({
        data: {
          ...
          thumbnail: 'thumbnail',
          description: 'description',
          deletedAt: new Date(), // 삭제된 캠페인 데이터
        },
      });
      await prismaService.user.create({
        data: {
